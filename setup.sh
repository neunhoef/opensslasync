...
# Create a ca key:
openssl genrsa -aes256 -out certificates/ca-key.pem 2048

# Inspect it:
openssl rsa -in certificates/ca-key.pem -text -noout

# Create a certificate from it:
openssl req -x509 -new -nodes -extensions v3_ca -key certificates/ca-key.pem -days 1024 -out certificates/ca-root.pem -sha512 -subj "/C=DE/ST=NRW/L=Kerpen/O=Neunhoeffer/OU=Max/CN=Max Neunhoeffer/emailAddress=max@9hoeffer.de/"
...
openssl x509 -req -in certificates/server-key.csr -days 3650 -signkey certificates/ca-key.pem -out certificates/server-crt.pem -extensions req_ext -extfile certificates/ssl.conf
...
openssl req -new -key certificates/client-key.pem -out certificates/client-req.pem -subj "/O=ArangoDB/CN=Max/"
...
# Create client cert:
openssl x509 -req -in certificates/client-req.pem -CA certificates/ca-root.pem -CAkey certificates/ca-key.pem -set_serial 101 -extensions client -days 365 -outform PEM -out certificates/client-crt.pem -extfile certificates/ssl.conf

# Inspect:
openssl x509 -in client-crt.pem -text -noout

# Export for browser:
openssl pkcs12 -export -inkey certificates/client-key.pem -in certificates/client-crt.pem -out certificates/client.p12
